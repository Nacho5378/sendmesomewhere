...
6. For live database reads, create a dedicated Supabase project, execute the migration and configure `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` as server-only values. Preserve `preview` campaign status and null timestamps.
