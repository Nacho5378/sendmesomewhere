...
function normalizeBase(payment,account){
 const paymentAccount=payment.account_id??payment.company?.id;
 if(paymentAccount!==account||payment.status!=='paid'||payment.substatus!=='succeeded'||payment.auto_refunded||payment.refunded_at)throw new Error('Payment is not eligible');
 ...
 if(!/^pay_[a-zA-Z0-9]+$/.test(payment.id)||!/^ch_[a-zA-Z0-9]+$/.test(payment.checkout_configuration_id)||!/^plan_[a-zA-Z0-9]+$/.test(planId)||!Number.isFinite(Date.parse(payment.paid_at)))throw new Error('Missing payment binding');
 return {paymentId:payment.id,checkoutId:payment.checkout_configuration_id,planId,amountCents:paymentCents(payment.subtotal,payment.currency),paidAt:payment.paid_at};
}
export function normalizePayment(payment,account){
 const base=normalizeBase(payment,account);
 if(!payment.metadata?.reservation_id)throw new Error('Missing sponsorship reservation');
 return {...base,reservationId:payment.metadata.reservation_id};
}
export function normalizeSupportPayment(payment,account){
 const base=normalizeBase(payment,account);
 if(!payment.metadata?.support_checkout_id)throw new Error('Missing support checkout');
 const supporter=payment.user_id??payment.user?.id??payment.member?.user?.id??null;
 const supporterKey=typeof supporter==='string'&&supporter.length<=120?supporter:null;
 return {...base,supportCheckoutId:payment.metadata.support_checkout_id,supporterKey};
}
export async function whopRequest(path,options={}){
 ...
 const res=await fetch(`https://api.whop.com/api/v1${path}`,{...fetchOptions,headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json','Api-Version-Date':API_VERSION,...fetchOptions.headers},signal:AbortSignal.timeout(8000),cache:'no-store'});
